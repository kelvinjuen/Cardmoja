<div class="row d-md-none" id="row-25">'+
    '<div class="col-12 text-center mb-1">'+
        '<h2 class="card-title"><span class="name">full_name</span></h2>'+
        '<h3 class="card-subtitle mt-1 colour_2"><span class="position">position</span>, <span class="company">Company</span></h3>'+
    '</div>'+
'</div>'+
'<div class="row" id="row-30">'+
    '<div class="col-sm-6 col-md-6 col-lg-6  d-none d-md-block">'+
        '<h3 class="card-title"><span class="name">full_name</span></h3>'+
        '<h3 class="card-subtitle mb-2 mt-2 colour_2"><span class="position">position</span></h3>'+
    '</div>'+
    '<div class="col-sm-6 col-md-6 col-lg-6 d-none d-md-block">'+
        '<h3 class="card-title text-right"><span class="company">Company</span></h3>'+
    '</div>'+
'</div>'+
'<div class="row border-top mt-2">'+
    '<div class="col-12 col-sm-8 col-md-9">'+
        '<div class="row" id="row-40">'+
            '<ul class="list-unstyled float-left  ml-2  d-none d-md-block mt-3 info"></ul>'+
            '<ul class="col-12 list-unstyled text-center text-sm-left d-md-none mt-1 info"></ul>'+
        '</div>'+
    '</div>'+
    '<div class="col-sm-4 col-md-3 py-4 d-none d-sm-block">'+
        '<img class="img-fluid  ml-auto   d-block"  id="profile-photo" alt="photo"/>'+
    '</div>'+
    '<div class="col-12 d-sm-none text-center mt-1">'+
        '<img class="img-fluid"  id="profile-photo-round" alt="photo"/>'+
        '<div class="row mt-2"><ul class="col-12 services colour_2"></ul></div>'+
    '</div>'+
'</div>'+
'<div class="row d-none d-sm-block text-center border-top"><ul class="col-12 services colour_2"></ul></div>'
